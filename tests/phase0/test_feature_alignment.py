...
def _item(pid: str, length: int) -> dict:
...
def test_long_mask_is_pair_mask_and_separation():
    with tempfile.TemporaryDirectory() as td:
        cache = Path(td)
        _write_cache(cache, "p1", 120)
        b = _collate([_item("p1", 120)], cache, crop_size=None)
        pm = b["pair_mask"][0].numpy()
        lm = b["long_mask"][0].numpy()
        n = pm.shape[0]
        sep = np.abs(np.arange(n)[:, None] - np.arange(n)[None, :])
        assert np.array_equal(lm, pm * (sep >= 6))
        assert np.all(np.diag(pm) == 0), "diagonal must be excluded from pair_mask"
